...
I started learning Elm in a quest for more reliability in web front end programming, as I stated in part 1. I implemented an algorithm, a parser, and it went very well.

Now let's build a simple application with user interaction. I want to see if I can still feel the same "reliability feeling" I felt when I was only implementing a function.

It turns out Elm has a standard way of structuring apps that's baked in the language, the Elm Architecture. It's very simple, and has *already* influenced a lot front end programming as a whole, mainly through the Redux library. We'll explore a simple version of the architecture in this post.

## The Spec
The specification for our web app is:
...
Next, running `elm package install` in the desired directory installs the language core, and creates a `elm-package.json` file. That file is the equivalent of `package.json` for the Elm world.

The easiest way to start developing is by editing the code in an editor (I've been using [VS Code](https://github.com/sbrink/vscode-elm) for Elm and it feels great), and run `elm-reactor` in the project directory.
...
After rendering, the application "waits" for a user interaction or any other event. When that happens, it triggers the update function. The update function is a function that receives the old model and data of the event, and returns a new model. This model is then rendered, and the loop continues.

Elm gives us a function that does all the "plumbing" for us, and it's called `Html.App.program`. We'll use a simpler version of it, because that's everything we need for our small app, and it's called `beginnerProgram`. With it, you only need to define your model, update and view and it does the heavy work for you:
```elm-lang
...
Our app now needs *two inputs* that work the same way. The first thing that pops in our head is making a component of our current app, and then show two of them. Let's do it.
...
Now let's how how to actually tag those messages in the new view:
...
The syntax is very restrictive, and this can be frustrating to some developers used to imperative programming. But as soon as I started changing code, refactoring, extracting components, changing specs, I could feel that the "restrictiveness" pays off. I feel that the language forces you to think a little bit more before writing code, and then makes refactoring easier and simpler.

##Conclusions

My initial objective when I started learning Elm was investigate ways to have more reliable front end code. Up until now everything is going smooth, I had no runtime error after the code got compiled, and I have a lot of trust that the code is doing what it's supposed to do. It's also still fun!

In part 3 I plan to make a simple application that makes http requests, and check if code is still simple as it's now.
